...
export class AgentRegistryService implements OnModuleInit {
  ...
  private async loadAgentProfiles() {
    ...
    for (const file of files) {
      try {
        ...
        console.error(`❌ Failed to load agent profile from ${file}:`, error.message);
      }
    }
  ...
  private async seedAgentDatabase() {
    ...
    for (const [name, profile] of agentEntries) {
      try {
        ...
        console.log(`✅ Seeded agent @${name} to database`);
      } catch (error) {
        console.error(`❌ Failed to seed agent ${name} to database:`, error.message);
      }
    }
  }
  ...
  // Hermes integration: delegate task to specific agent
  async delegateToAgent(agentName: string, task: string, context?: any): Promise<any> {
    const canSpawn = await this.spawnAgent(agentName, context);
    
    if (!canSpawn) {
      throw new Error(`Cannot spawn agent @${agentName} - not available or at capacity`);
    }

    const profile = this.getAgent(agentName);
    
    // This is where we would integrate with Hermes (you as Kiro)
    // For now, return the agent profile and task for Hermes to execute
    return {
      agent: profile,
      task,
      context,
      instructions: `You are now acting as @${agentName}. ${profile.prompt}\n\nTask: ${task}`,
    };
  }
}